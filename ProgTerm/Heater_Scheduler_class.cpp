...
void HeaterSchedulerCs:: EEPROMdel(int start_addr, int len)
{
  Log.Debug(F("\nEEPROM delete from "));Log.Debug(String(start_addr)); Log.Debug(F(" to")); Log.Debug(String(start_addr + len)); Log.Debug(F("\n"));
   for (int _add = start_addr; _add <= (start_addr + len); _add++) {
    EEPROM.write(_add, 0);
  }//delete
...
//    EEPROM.write(_add, 0);
//  }//delete
  EEPROMdel(addr,GetEEdaystructsize);
...
//    EEPROM.write(_add, 0);
//  }
 EEPROMdel(addr, GetEEeventstructsize);
...
void HeaterSchedulerCs::EEPromDefault()
{
  ...
  {
    Sched.Daily.dow = (timeDayOfWeek_t) _dow;
#ifdef debug
