...
#if defined(EEPROM_FLASH_PARTITION_NAME)
  Log.Debug("\nESP32 variant\n");
  EEPROM.begin(HS.EEPROM_size);
#endif
  ...
  switch (cmd)
  {
    ...
    case 3: //Read alarm eventDay
      ...
      if (CP.Nfield == 3)
      {
        ...
        HS.RdEEPROMday((timeDayOfWeek_t)_dow, _evnum);
        Log.Info(F("\n#3>"));
    ...
    case 6: //Change Mode
      ...
      {
        Log.Debug(F("\nCommand Change Mode: "));
        Mode_en _Mode = ( Mode_en) CP.Field[1]; Log.Verbose(String(_Mode)); Log.Verbose(",");
    ...
    case 7: //Manual on/off
      ...
      if (CP.Nfield == 2)
      {
        ...
        Heater_en _HeaterStat = (Heater_en) CP.Field[1]; Log.Verbose(String(_HeaterStat)); Log.Verbose(F("\n"));
        if (_HeaterStat == _ON)
        {
          Log.Debug(F("ON\n"));
          TurnON();
    ...
    case 8: //set alarm Once
      ...
      if (CP.Nfield == 7)
      {
        ...
        Log.Info(HS.SetEventOnce(0, (HS.SetTimeEvent(0, 0, 0, _Tgg, _Tmo, _Tyy) + tday), (EventState_sc) _en, (SwitchState_sc) _onoff, sw1)); Log.Info(F("\n"));
...
void UpdateDailySched()
{
  ...
  if (!HS.GetNextDayEv((timeDayOfWeek_t)dow))
  {
    //    Log.Verbose(F("\nSet alarm at="));
